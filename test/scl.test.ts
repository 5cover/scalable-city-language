...
const nodeDegrees = (network: BuildResult): Map<string, number> => {
    ...
    for (const segment of network.segments) {
        degrees.set(segment.startNodeKey, (degrees.get(segment.startNodeKey) ?? 0) + 1);
...
await test('spiral road generates multiple segments and increasing radius behavior', () => {
    const canvas = createCanvas({
        maxSegmentLength: 50,
    });
    ...
    const network = canvas.build();
    const orderedPoints = network.segments.map(segment => segment.start).concat(network.segments.at(-1)?.end ?? []);
...
await test('line and circle intersection creates shared nodes', () => {
    const canvas = createCanvas({
        maxSegmentLength: 200,
    });
...
await test('line and spiral intersection creates shared nodes', () => {
    const canvas = createCanvas({
        maxSegmentLength: 200,
    });
...
await test('segment subdivision respects maxSegmentLength', () => {
    const canvas = createCanvas({
        maxSegmentLength: 96,
    });
...
await test('omitted y does not emit y in XML', () => {
    const canvas = createCanvas({
        maxSegmentLength: 200,
    });
